...
def generate_multi_doc(draft: str, brief, primary_doc_type: str, mode: str):
    ...
    versions.append(DocVersion(
        doc_type=primary_doc_type, doc_type_name=name,
        content=draft, word_count=len(draft), generation_order=0,
    ))
    ...
    if short_type != primary_doc_type:
        ...
        versions.append(DocVersion(
            doc_type=short_type, doc_type_name=short_doc["name_cn"],
            content=short_content, word_count=len(short_content),
            generation_order=1, extracted_from=primary_doc_type,
        ))
    return versions
